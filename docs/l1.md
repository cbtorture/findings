**L1**: List initializer element evaluation ordering
====================================================
...
The order in which the elements in a list initializer are evaluated is unspecified.
Here we will be looking at a piece of code which results in different outcomes
depending on the order in which the elements in the list initializer are evaluated.
Seeing that there is implicit order in an array we will be able to see if the evaluation
order differs by the placement of returned results in the array:

```c
int result[2] = {counter(), counter()};
```

**L1** is very basic, we are using only two elements in the initializer for `result[2]` and
also the same funciton is being used, `counter()`, as the element initializer.
